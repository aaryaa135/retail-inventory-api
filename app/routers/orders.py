...
def create_order(order: schemas.OrderCreate, db: Session = Depends(get_db)):
    ...
    for item in order.items:
        if item.product_id in seen:
            raise HTTPException(status_code=400, detail=f"Duplicate product_id {item.product_id} in order")
    ...
    for item in order.items:
        ...
        inv = db.query(models.Inventory).filter(models.Inventory.product_id == item.product_id).with_for_update().first()
        if not inv:
    ...
    return schemas.OrderResponse(
        id=db_order.id, status=db_order.status, total_amount=float(db_order.total_amount),
        items=[schemas.OrderItemResponse(id=i.id, product_id=i.product_id, quantity=i.quantity,
               unit_price=float(i.unit_price), subtotal=float(i.unit_price) * i.quantity) for i in db_order.items],
...
def list_orders(db: Session = Depends(get_db)):
    ...
    return [schemas.OrderResponse(
        ...
        items=[schemas.OrderItemResponse(id=i.id, product_id=i.product_id, quantity=i.quantity,
               unit_price=float(i.unit_price), subtotal=float(i.unit_price) * i.quantity) for i in o.items],
...
def get_order(order_id: int, db: Session = Depends(get_db)):
    ...
    return schemas.OrderResponse(
        id=order.id, status=order.status, total_amount=float(order.total_amount),
        items=[schemas.OrderItemResponse(id=i.id, product_id=i.product_id, quantity=i.quantity,
               unit_price=float(i.unit_price), subtotal=float(i.unit_price) * i.quantity) for i in order.items],
...
def cancel_order(order_id: int, db: Session = Depends(get_db)):
    order = db.query(models.Order).filter(models.Order.id == order_id).with_for_update().first() if False else db.query(models.Order).filter(models.Order.id == order_id).first()
