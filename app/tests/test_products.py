class TestCreateProduct:
    ...
    def test_create_product_without_category(self, client):
        r = client.post(
            "/api/v1/products/", json={"name": "Generic", "sku": "GI-001", "price": 9.99}
        )
        assert r.status_code == 201
    ...
    def test_invalid_category_returns_404(self, client):
        r = client.post(
            "/api/v1/products/",
            json={"name": "X", "sku": "X-001", "price": 10.0, "category_id": 9999},
        )
...
class TestGetProducts:
    ...
    def test_search_by_name(self, client):
        ...
        client.post(
            "/api/v1/products/", json={"name": "Apple Watch", "sku": "AW-001", "price": 399.0}
        )
    ...
    def test_pagination(self, client):
        for i in range(5):
            client.post(
                "/api/v1/products/", json={"name": f"Item {i}", "sku": f"IT-00{i}", "price": 10.0}
            )
