# Retail Inventory API — Complete Code Explanation (Whole Project)
...
```
Client → FastAPI (app/main.py:32, lifespan+ CORS) → Routers → SQLAlchemy ORM (app/models.py) → Postgres (or sqlite)
                                          ↘ Pydantic Schemas (app/schemas.py) for validation
                                          ↘ pytest + TestClient (app/tests/conftest.py:23) overrides get_db
```
...
│   ├── main.py              # FastAPI app, lifespan retry, CORS, routers, HTML landing at / + /health
│   ├── database.py          # engine, SessionLocal, Base, get_db(), DATABASE_URL + sqlite check_same_thread
│   ├── models.py            # 5 tables + OrderStatus enum, Numeric(10,2), ondelete, cascade
│   ├── schemas.py           # Pydantic v2 DTOs, Field validation, ConfigDict
│   ├── routers/
│   │   ├── __init__.py
│   │   ├── categories.py    # CRUD, 409 dup, 409 delete guard
│   │   ├── products.py      # CRUD + ?search/?category_id/?skip/limit + 409 guards
│   │   ├── inventory.py     # 1:1 product, is_low_stock, /low-stock sorted
│   │   └── orders.py        # POST with_for_update + dedup 400, 422 stock, cancel restore
│   └── tests/
│       ├── __init__.py
│       ├── conftest.py      # TestClient + override_get_db, fixtures sample_category→product→inventory
│       ├── test_categories.py # 11 tests
│       ├── test_products.py   # 17 tests
│       ├── test_inventory.py  # 13 tests
│       └── test_orders.py     # 11 tests
├── alembic/
│   ├── env.py               # reads DATABASE_URL, imports models for autogenerate
│   ├── script.py.mako
│   └── versions/001_initial.py # creates all 5 tables
├── conftest.py              # sys.path fix for Windows "No module named app"
├── pytest.ini               # testpaths = app/tests, pythonpath = .
├── requirements.txt         # pinned: fastapi 0.115, sqlalchemy 2.0.35, pydantic 2.9.2, alembic 1.13.3, etc.
├── pyproject.toml           # black/ruff/pytest/cov config
├── Dockerfile               # python:3.11-slim, pip install, CMD sh -c uvicorn ... ${PORT:-8000}
├── docker-compose.yml       # db:5432 postgres:16-alpine healthcheck + api:8001:8000 restart unless-stopped
├── Makefile                 # setup/test/run/migrate/docker-up (cross-platform)
├── .env.example             # DATABASE_URL template
├── .dockerignore
├── alembic.ini
├── render.yaml              # Render blueprint (now sqlite fallback, no bank)
├── .github/workflows/ci.yml # ruff + black --check + pytest --cov
├── LICENSE (MIT) + README.md (mermaid) + docs/CODE_EXPLANATION.md (this file)
└── .gitignore / setup.bat / start_server.bat / run_tests.bat
```
...
### `conftest.py:1-6` — Windows path fix
```python
...
### `requirements.txt:1-10` — pinned prod = reproducible
```
fastapi==0.115.0
...
### `app/database.py:1-26` — engine + session

* `load_dotenv()` `app/database.py:5` → reads `.env`
* `DATABASE_URL = os.getenv(..., "sqlite:///./retail_inventory.db")` `app/database.py:9` — Docker sets `postgresql://retail:retail@db:5432/retail` `docker-compose.yml:22`
* `connect_args = {"check_same_thread": False} if sqlite else {}` `app/database.py:14` — required for SQLite
...
* `get_db()` `app/database.py:21` — FastAPI `Depends` generator, `yield db` then `close()`

### `app/models.py:1-66` — 5 tables
...
```
* `Numeric(10,2)` `app/models.py:29` — money, not `Float` (production fix)
* `ondelete=SET NULL` `app/models.py:30` — deleting category doesn't cascade delete products
...
* `ondelete=CASCADE` `app/models.py:41` — product deleted → inventory deleted
...
```
* `native_enum=False` `app/models.py:51` — works on SQLite
...
```
* `RESTRICT` `app/models.py:62` — can't delete product if orders exist → router maps to `409`

### `app/schemas.py:1-102` — Pydantic v2 DTOs
...
### `app/main.py:1-59` — FastAPI app

* `lifespan` `app/main.py:10-29` — `if AUTO_CREATE_TABLES=="true"` → `Base.metadata.create_all(bind=engine)` with 10× retry on `could not translate host name "db"` (Docker DNS race) else `print` and don't block `health`
...
* `GET /` `app/main.py:53` → `HTMLResponse` landing page (crazy README-style HTML with Swagger buttons, feature cards, endpoint table) — so live link looks pro. `GET /health` `app/main.py:58` → `{"status":"healthy"}`

### `app/routers/products.py:1-72`

* `POST /` `app/routers/products.py:10` — check `sku` dup `409`, `category_id` 404, `Product(**product.model_dump())` (was `dict()`), commit
* `GET /` `app/routers/products.py:24` — `skip ge0, limit 1-100, category_id, search` → `ilike(f"%{search}%")` on name|sku, `offset/limit`
* `GET /{id}` `404` if not found
* `PUT /{id}` `app/routers/products.py:51` — check `category_id`, `updates.model_dump(exclude_unset=True)`
* `DELETE /{id}` `app/routers/products.py:66` — check `OrderItem.product_id` → `409` else try `delete` + `except→409`
...
* `GET /` list, `GET /{id}` 404
* `PUT /{id}` update
* `DELETE /{id}` `app/routers/categories.py:47` — check `Product.category_id` → `409`, else try delete

### `app/routers/inventory.py:1-80`

* `POST /` `app/routers/inventory.py:10` — `404` product, `409` inventory exists, `Inventory(**inv.model_dump())`, set `is_low_stock = qty <= threshold`
* `GET /low-stock` `app/routers/inventory.py:25` — **must be before `/{product_id}`** (order matters), joins `Inventory+Product`, `threshold_override` optional, `if qty <= threshold` → `LowStockAlert(product_id, product_name, sku, current_quantity, threshold, shortage=max(0,thr-qty))`, `sorted by current_quantity`
* `GET /` `app/routers/inventory.py:48` — all, compute `is_low_stock`
* `GET /{product_id}` `404` if no inventory
* `PUT /{product_id}` `70` — `updates.model_dump(exclude_unset)`, recompute `is_low_stock`

### `app/routers/orders.py:1-93`

* `POST /` `app/routers/orders.py:10` — **dedup** `seen set` → `400 duplicate product_id`, loop: `Product` check `404`, `Inventory with_for_update()` `app/routers/orders.py:19` → `400 no inventory`, `422 insufficient`, `subtotal=float(price)*qty`, `total_amount` sum, `db_order=Order(total_amount, CONFIRMED)` `flush`, then `OrderItem` + `inv.quantity-=qty`, `commit`, return `OrderResponse` with `float()` casts for `Numeric`
* `GET /` `50` — all, float casts
* `GET /{id}` `61` — 404, float casts
* `PATCH /{id}/cancel` `74` — `Order` (with_for_update no-op on SQLite), `400 already cancelled`, `Inventory with_for_update` per item `+=qty`, `CANCELLED`, commit, float casts

### `app/tests/conftest.py:1-51` — test wiring
...
* `client` fixture `scope=function` `app/tests/conftest.py:22` — `create_all(bind=test_engine)`, `app.dependency_overrides[get_db]=override_get_db`, `with TestClient(app) as c: yield c`, then `drop_all`, `clear()`
* `sample_category` `32` — `POST /categories/ {"name":"Electronics"}`
* `sample_product` `38` — `POST /products/ {"Wireless Headphones","WH-001",299.99, category_id}`
* `sample_inventory` `47` — `POST /inventory/ {"product_id", qty 50}`

### `app/tests/test_categories.py:1-51` (11 tests) — was missing
...
Monies: `Numeric(10,2)` everywhere — interview win over `Float`.
...
**Order creation flow:**
1. Dedup `product_id` in payload `orders.py:13` → 400
2. For each item: lock `Inventory` row `with_for_update()` `19` (Postgres `SELECT FOR UPDATE`, SQLite no-op)
3. Check stock `if inv.quantity < qty` → 422, else `total_amount += float(price)*qty`
...
**Low-stock:**
* `GET /low-stock` scans `Inventory join Product` `30`, uses `threshold_override` or `low_stock_threshold`, `if qty <= thr` → alert, `shortage = thr - qty`, `sorted(qty asc)` `45`
...
* Run: `pytest app/tests/ -v` → `52 passed, 0 warnings` (was 6, fixed `ConfigDict` + `declarative_base`)
...
* `alembic/versions/001_initial.py:1` creates 5 tables with `Numeric`, `Enum`, `ondelete` — `upgrade/downgrade`
* `app/main.py:14` `lifespan` does `create_all` with 10× retry on `could not translate host name "db"` (Docker DNS race). For prod: `AUTO_CREATE_TABLES=false` + `alembic upgrade head`; for dev: `true` auto-creates. Stamped: `alembic stamp head` → `001 (head)`
...
* **Why `lifespan` not `create_all` at import?** `app/main.py:10` — prod uses Alembic, dev fallback with retry.
...
*End. For live demo: `docker compose up --build` → `http://localhost:8001/` (HTML) → `/docs` try-it-out. MIT © 2026.*
